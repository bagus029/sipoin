...
class CrudCode extends CCodeModel
{
	...
	public function generateActiveField($modelClass,$column,$htmlOptions = 'form-control')
	{
		if($column->type==='boolean')
			return "\$form->checkBox(\$model,'{$column->name}')";
		elseif(stripos($column->dbType,'text')!==false)
			return "\$form->textArea(\$model,'{$column->name}',array('class'=>'textarea'),array('rows'=>6, 'cols'=>50))";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$model,'{$column->name}',array('class'=>'".$htmlOptions."'))";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array('class'=>'".$htmlOptions."'),array('size'=>$size,'maxlength'=>$maxLength))";
			}
		}
	}
