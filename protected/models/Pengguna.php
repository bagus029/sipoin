...
class Pengguna extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('idpengguna, level, status', 'numerical', 'integerOnly'=>true),
			array('username', 'length', 'max'=>35),
			array('password', 'length', 'max'=>200),
			array('namapengguna, jabatan', 'length', 'max'=>85),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('idpengguna, username, password, namapengguna, jabatan, level, status', 'safe', 'on'=>'search'),
		);
	}
